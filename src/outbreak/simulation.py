...
def run_simulation(
    ...
    return SimulationSummary(
        ...
        average_final_population=_average(
            result.final_population for result in results
        ),
...
def simulate_matchup(
    ...
    if pathogen_names:
        pathogen_pool = [
            card for card in pathogen_pool if card.name in set(pathogen_names)
        ]
...
def _play_pathogen_turn(game: GameState, random: Random) -> None:
    ...
    if game.active_player.actions_remaining:
        ...
        if population >= 4 and targets and random.random() < 0.25:
            ...
            game.perform_action(
                "replicate", pathogen_name=pathogen_name, location=location
            )
        _pass_response(game)
    game.end_turn()
...
def _play_host_turn(game: GameState, random: Random) -> None:
    ...
    elif _can_play_host_card(game, "Antiviral") and _location_has_pathogen_class(
        game, location, "virus"
    ):
    ...
    if game.active_player.actions_remaining:
        ...
        if _can_play_host_card(game, "PCR") and game.board.total_population() == 0:
            ...
            and _can_play_host_card(game, "Antiviral")
            and _location_has_pathogen_class(game, location, "virus")
        ):
            game.perform_action("treat", card_name="Antiviral", location=location)
        elif (
            location is not None
            and _can_play_host_card(game, "Antibiotic")
            and _location_has_pathogen_class(game, location, "bacterium")
        ):
            game.perform_action("treat", card_name="Antibiotic", location=location)
...
def _pathogen_location(game: GameState, random: Random) -> Location:
    locations = [
        location for location, state in game.board.locations.items() if state.population
    ]
...
def _most_populated_location(game: GameState, random: Random) -> Location | None:
    ...
    return random.choice(
        [location for location, population in populated if population == maximum]
    )


def _pathogen_name(game: GameState, random: Random) -> str | None:
    available = [
        card.name for card in game.pathogen.hand if card.name in game._pathogen_cards
    ]
    return (
        random.choice(available)
        if available
        else next(iter(game._pathogen_cards), None)
    )


def _can_play_host_card(game: GameState, card_name: str) -> bool:
    return any(
        card.name == card_name and card.cost <= game.host.energy
        for card in game.host.hand
    )


def _location_has_pathogen_class(
    game: GameState, location: Location, pathogen_class: str
) -> bool:
    return any(
...
def main() -> None:
    ...
    parser.add_argument(
        "--max-rounds", type=int, default=50, help="round limit per game"
    )
